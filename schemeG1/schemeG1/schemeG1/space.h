...
public:
	...
	space(const space& s);
	~space();
	void showlog(ofstream& logfile) const;
	...
	void fillspace(const string& cellfname, const string& initvalfname, const char* option);
	void fillspace(const string& ivfname);
	void fillsomecells(int cellsnumber, int& ix, int& iy, int& iz);
	void skipsomecells(int cellsnumber, int& ix, int& iy, int& iz);
	...
	void saveslice(int i1, int i2, int step);
