...
/*-------------------------------riemi--------------------------------*/
       ...
       int k,i,it;
       double rc_1,e_pmin,u_vc,p0,p1,pp,uu,ai,tmp1,tmp2;
...
/*     wave coeff. and dens   */
       ...
       for ( i=0; i <= 1; i++ )
         ...
         if(p0 < rqp[2][i]*estr)
           ...
           cc[i]=c[i]-0.5*ai*(g-1.)*(rqp[1][i]-uu);
           ss[1][i]=uu+ai*cc[i];
           rr[i]=g*pp/(cc[i]*cc[i]);
           }
         else
           {
           tmp1=(g+1.)*pp+(g-1.)*rqp[2][i];
           tmp2=(g-1.)*pp+(g+1.)*rqp[2][i];
...
/*           2          */
       if( ( om > ss[0][0] ) && ( om <= ss[1][0]) )
         {
         tmp1=1./(g+1.);
         cc[0]=(2.*c[0]+(g-1.)*(rqp[1][0]-om))*tmp1;
         u_o=om+cc[0];
         r_o=rqp[0][0]*pow( (cc[0]/c[0]),gc );
         p_o=cc[0]*cc[0]*r_o/g;
         goto rtn_o;
         }
/*          3           */
...
/*          5           */
       if( (om > ss[1][1]) && (om <= ss[0][1]) )
         {
          tmp2=1./(g+1.);
          cc[1]=(2.*c[1]+(g-1.)*(om-rqp[1][1]))*tmp2;
          u_o=om-cc[1];
          r_o=rqp[0][1]*pow( (cc[1]/c[1]),gc );
          p_o=cc[1]*cc[1]*r_o/g;
          goto rtn_o;
         }
/*        6             */
